...
export async function getBlogBySlug(slug) {
  return await client.fetch(
    ...
    *[_type == "blog" && slug.current == $slug][0] {
      ...
      slug,
      excerpt,
      details,        // main portable text content
      keywords,
      tags,
      author,
      publishedAt,
...
export async function createBooking(bookingData) {
  try {
    const bookingIdRan = Math.floor(100000 + Math.random() * 900000);
    const bookingId = `LCW-${bookingIdRan}`;
    const booking = {
      _type: 'booking',
      bookingId,
      ...
      carDetails: {
        name: bookingData.carDetails?.name || 'N/A',
        seats: bookingData.carDetails?.seats || '',
        fuelType: bookingData.carDetails?.fuelType || '',
        category: bookingData.carDetails?.category || '',
      },
      pricing: {
        baseFare: bookingData.pricing?.baseFare || 0,
        deliveryPickup: bookingData.pricing?.deliveryPickup || 0,
        insuranceGst: bookingData.pricing?.insuranceGst || 0,
    ...
    const result = await clientAPI.create(booking);
...
export async function updateBookingStatus(bookingId, status, notes = '') {
  try {
    ...
    const result = await clientAPI.patch(booking._id).set(updateData).commit();
...
export async function createContact(contactData) {
  try {
    ...
    const result = await clientAPI.create(contact);
...
export async function updateContactStatus(contactId, status, notes = '') {
  try {
    ...
    const result = await clientAPI.patch(contact._id).set(updateData).commit();
